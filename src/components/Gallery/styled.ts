...
export const Grid = styled.div`
  ...
  ${media.medium`
    grid-template-rows: repeat(${GALLERY_COLS_MEDIUM}, 1fr);
    grid-template-columns: repeat(${GALLERY_ROWS_MEDIUM}, 1fr);
    grid-row-gap: ${GALLERY_ROW_GAP_MEDIUM}px;
    grid-column-gap: ${GALLERY_COL_GAP_MEDIUM}px;
  `}

  ${media.large`
    grid-template-rows: repeat(${GALLERY_COLS_LARGE}, 1fr);
...
export const Container = styled.section<{ isHidden: boolean }>`
  ...
  height: calc(100vh - ${NAV_HEIGHT + 80}px); // account for padding
`;
